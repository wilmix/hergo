...
class ReportIngreso extends CI_Controller {
  public function index($ini,$fin,$alm,$tin) {
      //CARGAR MODELO
        $alm = $alm == 0 ? '' : $alm;
        $tin = $tin == 0 ? '' : $tin;
      $this->load->model('Reportes_model');
      $lineas = $this->Reportes_model->mostrarReporteIngreso($ini,$fin,$alm,$tin)->result();
    ...
    $this->load->library('ReportIngresos_lib', $params);

      $this->pdf = new ReportIngresos_lib($params);
      ...
      //guardar
      $this->pdf->Output('Reporte Ingresos', 'I');
  }
